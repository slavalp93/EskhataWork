  ...
  partial class ContractFunctions
  {
    ...
    public List<string> ImportContractsFromXml()
    {
      ...
      try
      {
        ...
        var documentElements = xDoc.Descendants("Data").Elements("Document").ToArray();
        ...
        foreach (var dataElement in documentElements)
        {
          var documentElement = dataElement.Element("Document");
          var counterpartyElement = dataElement.Element("Counterparty");
    ...
    public static Sungero.Docflow.IAddendum CreateLegalOpinion()
    {
      var aviabledDocumentKinds =
        Sungero.Docflow.PublicFunctions.DocumentKind.GetAvailableDocumentKinds(typeof(Sungero.Docflow.IAddendum));
    ...
    public override StateView GetDocumentSummary()
    {
      ...
      if (_obj.RegistrationDate != null)
        documentName += Sungero.Docflow.OfficialDocuments.Resources.DateFrom +
      ...
      if (_obj.Counterparty != null)
      {
        ...
        if (_obj.Counterparty.Nonresident == true)
          documentBlock.AddLabel(string.Format("({0})", _obj.Counterparty.Info.Properties.Nonresident.LocalizedName)
                                 .ToLower());
      }
      else
      {
      ...
      // Валюта.
      var currencyText = string.Format("{0}: {1}", _obj.Info.Properties.CurrencyContractlitiko.LocalizedName,
      ...
      var validFrom = _obj.ValidFrom.HasValue
        ? string.Format("{0} {1} ", Sungero.Contracts.ContractBases.Resources.From,
                        _obj.ValidFrom.Value.Date.ToShortDateString())
        : string.Empty;

      var validTill = _obj.ValidTill.HasValue
        ? string.Format("{0} {1}", Sungero.Contracts.ContractBases.Resources.Till,
