...
namespace litiko.Eskhata.Client
{
  partial class CompanyActions
  {
    public virtual void FillFromABSlitiko(Sungero.Domain.Client.ExecuteActionArgs e)
    {
      ...
      var integrationMethodName = "R_DR_GET_COMPANY";            
      ...
      if (successed)
      {                
        var errorList = litiko.Integration.PublicFunctions.Module.Remote.R_DR_GET_COMPANY(exchDocId, _obj);
        if (errorList.Any())
    ...
    {
      return _obj.AccessRights.CanUpdate() && _obj.IsCardReadOnly != true && !_obj.State.IsChanged;
    }
