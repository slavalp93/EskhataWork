      ...
      {                        
        ...
        {                  
          ...
          {            
            ...
            builder.ParagraphFormat.LeftIndent = textIndentPt;              
            builder.ParagraphFormat.FirstLineIndent = -textIndentPt;
            foreach (var decigion in resolutionInfo.Decigions.Split(new[] { Environment.NewLine }, StringSplitOptions.RemoveEmptyEntries))
            {                                              
              builder.Writeln(decigion);
            }
            builder.ListFormat.RemoveNumbers();
    ...
    private void AddStringsToNumberedList(DocumentBuilder builder, IList<string> strings)
    {
        ...
        for (int i = 0; i < strings.Count; i++)
        {
            ...
            else
            {
                ...
                /*
                // Удаляем все оставшиеся Runs в текущем параграфе после записи
                var currentPara = builder.CurrentParagraph;
                bool remove = false;
    
                foreach (var node in currentPara.GetChildNodes(NodeType.Run, true).Cast<Run>().ToList())
                {
                    if (remove)
                    {
                        node.Remove();
                    }
                    else if (node.Text.Contains(strings[i]))
                    {
                        // Нашли Run с вставленным текстом — начинаем удаление следующих
                        remove = true;
    
                        // Если текст вставки короче, чем Run — удаляем остаток
                        int index = node.Text.IndexOf(strings[i]) + strings[i].Length;
